...
Status: accepted, 2026-09-23 (dictation).
...
- The menu bar menu's state line shows the running state, every session failure and a
  missing key. The overlay and settings milestones take this over.
- `api.x.ai` answers a wrong key with 400 (`"Incorrect API key provided"`) and sends
  ...
  to HTTP, and the menu words both `.badRequest` and `.unauthorized` as a key problem.

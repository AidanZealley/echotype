...
- Verification:
  ...
  - `codesign --verify --verbose=2` reports the bundle valid and satisfying its designated requirement. The authority is `Apple Development: aidan@zealley.com (3MQK9CND62)` and the DR includes the bundle identifier.
...
- Evidence checked:
  ...
  - `codesign --verify --verbose=2` reports `/Applications/EchoType.app` valid and satisfying its DR. The DR names the bundle identifier and the `Apple Development: aidan@zealley.com (3MQK9CND62)` leaf, and it is not ad hoc.

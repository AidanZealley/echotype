...
- Verification run:
  ...
  - `codesign --verify --verbose=2 /Applications/EchoType.app` reports valid on disk and satisfies its designated requirement. The DR names `com.aidanzealley.echotype` and the leaf `Apple Development: aidan@zealley.com (3MQK9CND62)`. It is not ad hoc.

...
Build-order step 5 of [the specification](../../specs/echotype-v1.md): the settings window,
the API key's write path into the Keychain, and launch at login. Today the defaults in
`Settings()` are the settings, the key is seeded by hand with `security`, and keyterms are
a hard-coded list. After this milestone every setting the window shows persists across
relaunches, the key is entered in the window, and one Test click validates the microphone,
the device choice, the key and the socket.

This milestone also owes the settings encoding a round-trip test. See
...
running the bare executable makes macOS treat the result as a new app. It then asks for
every TCC grant again, and it cannot read the Keychain item without a prompt.
...
Agents can compile, unit-test `EchoTypeCore` and read code. They cannot open the settings
window, read or write the real Keychain as the signed app, register a login item, see TCC
status or pick an audio device. Two external validation gates cover that:

- G1, at the end of workstream 1: Aidan checks that settings and the key persist and
  that the hotkey follows the dropdown.
- G2, at the end of workstream 2: Aidan checks the device picker, the Test button, the
  permission rows and launch at login.

Treat compile-and-review as the bar for everything outside `EchoTypeCore`. Do not build
fakes for `UserDefaults`, the Keychain, `SMAppService`, Core Audio, `AVAudioEngine` or
TCC to get a green suite, because a test of a fake only tests the fake.
...
2. [The approved specification](../../specs/echotype-v1.md). Most relevant are "Menu bar
   and settings", "Why Opt+D", "Architecture", "Permissions, signing and distribution"
   and "Build order".
3. [The decision records](../../decisions/README.md). Where a record and the
   specification disagree, the record is newer and wins.
4. [plan.md](plan.md), including its decision and drift log. Entries there approved by
   Aidan before the workflow started override the specification.
...
One integration branch, `feat/settings`, cut from the approved HEAD and recorded in
`plan.md`.
...
`Settings 01: persistent settings and the window`.

Changes stay uncommitted through implementation, review, remediation and the gate, so
the reviewer sees one coherent diff. No document records an accepted workstream's commit
hash.
...
```text
Implement workstream <N> of the settings milestone.

Read <packet path>, docs/settings/implementation/README.md and the source-of-truth
...
thin enough to read in one pass. Do not build fakes for UserDefaults, the Keychain,
SMAppService, Core Audio, AVAudioEngine or TCC to produce a test.
...
```text
Independently review workstream <N> of the settings milestone.
...
AppKit, SwiftUI, Security, Core Audio and ServiceManagement paths as carefully as
untested production code, because that is what they are. In this milestone, pay
particular attention to a stored value that could decode to a different hotkey after an
upgrade, to blocking Keychain or Core Audio calls on the main actor (the event tap runs
there), and to anything that could let a settings change or the Test button disturb a
running dictation. Tests that assert implementation details, or that exercise a fake the
...
```text
Run focused closure review on workstream <N> of the settings milestone.
...
```text
Lead workstream <N> of the settings milestone.

Read docs/settings/implementation/README.md and <NN-workstream.md>, then the
...
pass, then run focused closure in a fresh review session using the same brief. Then run
your packet's external validation gate before accepting.
...
```text
Lead the whole-feature review of the settings milestone.

Read docs/settings/implementation/README.md and final-review.md, then the
...
another round of hand verification unless a correction you accept touches the settings
encoding, the Keychain calls, the hotkey path, audio device selection or the start and
commit path.
...
Two gates, both needing Aidan at the Mac, both after focused closure and before
acceptance, because each proves a finished candidate. See the External validation gates
section of [plan.md](plan.md) for each gate's candidate and required evidence.

A gate that needs the user stops the lead. The lead records the candidate and the exact
steps in its workstream record's External validation section, writes an escalation entry
in `plan.md`, sets its row to `Blocked` and the gate to `Testing`, and returns the
escalation id with the work uncommitted.
...
When Aidan reports a failure the lead can diagnose, the lead enters a troubleshooting
loop inside the workstream. It records the candidate, the failure and the evidence,
makes the smallest correction, runs the agent-accessible checks, and blocks again for
Aidan to rerun the failed steps. None of that changes the workstream status or starts a
new implementation and review loop. Reopen the full loop only when a correction changes
approved behaviour, architecture, ownership, the stored settings format, how the key is
stored, or another accepted workstream. After the gate passes, the lead has any
meaningful unreviewed correction reviewed once before accepting.

A defect that G2 finds in a file workstream 1 owns may be corrected by the workstream 2
lead as a troubleshooting correction, provided it does not change a frozen contract in
the plan. A defect in a frozen contract is an escalation.
...
List the decisions from this milestone that belong in new decision records when the
workflow is retired into `docs/decisions/`, and the decision record lines this milestone
has made stale.
